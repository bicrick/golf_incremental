...
func _run() -> void:
	...
	ok = await _test_collect_hidden_cursor_when_view_ready() and ok
	ok = await _test_pan_grab_overrides_hidden() and ok
	ok = await _test_pan_end_restores_hidden() and ok
	print("cursor_ok=", ok)
	quit(0 if ok else 1)
...
func _test_collect_hidden_cursor_when_view_ready() -> bool:
	...
	CursorManager.refresh()
	if CursorManager.debug_applied_kind() != &"hidden":
		print("FAIL: collect mode with harvest view ready should hide OS cursor")
		main.queue_free()
		return false
	main.queue_free()
	print("OK: collect_hidden_cursor_when_view_ready")
	return true
...
func _test_pan_grab_overrides_hidden() -> bool:
	...
	CursorManager.refresh()
	if CursorManager.debug_applied_kind() != &"hidden":
		print("FAIL: pre-pan setup should hide cursor")
		main.queue_free()
		return false
	...
	if CursorManager.debug_applied_kind() != &"grab":
		...
		return false
	main.queue_free()
	print("OK: pan_grab_overrides_hidden")
	return true
...
func _test_pan_end_restores_hidden() -> bool:
	...
	if CursorManager.debug_applied_kind() != &"hidden":
		print("FAIL: ending pan drag should restore hidden cursor in collect mode")
		main.queue_free()
		return false
	main.queue_free()
	print("OK: pan_end_restores_hidden")
	return true
