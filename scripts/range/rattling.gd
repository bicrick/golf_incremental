...
func start(litter: Sprite3D, walk_speed: float, atmosphere_tint: Color) -> void:
	...
	_ball_credited = litter.get_meta("leftover_credited", false)
...
func _complete() -> void:
	if _carrying_ball:
		GameState.credit_rattling_ball(
			_ball_quality, _ball_yardage, _ball_golden, _ball_source, _ball_credited
		)
	finished.emit(self)
