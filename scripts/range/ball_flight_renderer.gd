...
class FlightConfig:
	...
	var arc_min_px: float = 24.0
	var arc_max_px: float = 80.0
	var landing_scatter_x: float = 28.0
	var range_x_min: float = 24.0
	var range_x_max: float = 456.0
	var flight_time_min_sec: float = 0.40
	var flight_time_max_sec: float = 2.80
	var flight_time_arc_sec: float = 0.55
	var flight_time_travel_sec: float = 1.10
	var flight_travel_ref_px: float = 110.0
...
static func build_path(
	...
	path.yards = yards
	path.persp_p = resolve_visual_p(yards, timing_tier, config, contact_flavor)
	path.arc_height = arc_height_for_hit(
		yards, timing_tier, stats, config, path.persp_p, contact_flavor
	)
	...
	path.flight_time = flight_time_for_hit(path, config)
	return path


static func flight_time_for_hit(path: FlightPath, config: FlightConfig) -> float:
	var tee := Vector2(config.tee_x, config.tee_y)
	var ground_travel := tee.distance_to(path.landing_ground)
	var arc_norm := sqrt(path.arc_height / maxf(config.arc_max_px, 1.0))
	var travel_norm := ground_travel / maxf(config.flight_travel_ref_px, 1.0)
	var time := config.flight_time_min_sec \
		+ config.flight_time_arc_sec * arc_norm \
		+ config.flight_time_travel_sec * travel_norm
	return clampf(time, config.flight_time_min_sec, config.flight_time_max_sec)
...
static func arc_height_for_hit(
	...
	var persp_p := visual_p if visual_p >= 0.0 else resolve_visual_p(
		yards, timing_tier, config, contact_flavor
	)
	...
	match contact_flavor:
		Balance.ContactFlavor.SLIGHTLY_FAT:
			base_arc *= 0.78
		Balance.ContactFlavor.THIN:
			base_arc = minf(base_arc, Balance.VISUAL_DRIBBLE_MAX_PX * 0.35)
		Balance.ContactFlavor.CHUNK:
			base_arc = minf(base_arc, 10.0)
	if timing_tier != Balance.TimingTier.MISS:
