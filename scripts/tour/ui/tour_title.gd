class_name TourTitle
extends Control
## v8 title: the logo over the live range, Continue / New game.
...
func _ready() -> void:
	...
	add_child(_logo)
	_sub = TourUi.outlined(TourUi.label("THE LONGEST HOLE", 8, TourUi.PAPER_HI), TourUi.INK, 3)
	_sub.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_sub.position = Vector2(0, 122)
	_sub.size = Vector2(480, 10)
	add_child(_sub)
	var vb := VBoxContainer.new()
	vb.position = Vector2(180, 160)
	...
	_cont_btn.visible = Tour.has_started()
	if not Tour.has_started():
		_new_btn.text = "Play"
...
func _on_new() -> void:
	if Tour.has_started() and not _confirm:
		_confirm = true
