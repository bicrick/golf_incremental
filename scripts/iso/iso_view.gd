...
class_name IsoView
extends Node2D
## DEPRECATED for player harvest — the live loop uses RangeView's 3D ortho
## camera (ViewModeController + RangeCameraController + PickupController).
## Kept in the scene tree (Mode.OFF / visible=false) for I-key build and
## verify_iso_view. Do not delete this scene or its scripts.
##
## Standalone 2D 45° isometric build + harvest view. Sibling of RangeView.
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if _mode == Mode.HARVEST:
		## Desktop Space returns to strike. Empty click / tap stays in harvest.
